#include "imu.h"

#include <avr/io.h>
#include <util/delay.h>
#include <avr/pgmspace.h>	// Pour stocker le fichier de configuration dans la flash

#include "twi.h"

// ----------------------------------------------------------------------------
// Inlcusions de parties du fichier bmi2_defs.h
// ----------------------------------------------------------------------------

/*! @name BMI2 register addresses */
#define BMI2_CHIP_ID_ADDR                             UINT8_C(0x00)
#define BMI2_STATUS_ADDR                              UINT8_C(0x03)
#define BMI2_AUX_X_LSB_ADDR                           UINT8_C(0x04)
#define BMI2_ACC_X_LSB_ADDR                           UINT8_C(0x0C)
#define BMI2_GYR_X_LSB_ADDR                           UINT8_C(0x12)
#define BMI2_SENSORTIME_ADDR                          UINT8_C(0x18)
#define BMI2_EVENT_ADDR                               UINT8_C(0x1B)
#define BMI2_INT_STATUS_0_ADDR                        UINT8_C(0x1C)
#define BMI2_INT_STATUS_1_ADDR                        UINT8_C(0x1D)
#define BMI2_SC_OUT_0_ADDR                            UINT8_C(0x1E)
#define BMI2_SYNC_COMMAND_ADDR                        UINT8_C(0x1E)
#define BMI2_GYR_CAS_GPIO0_ADDR                       UINT8_C(0x1E)
#define BMI2_INTERNAL_STATUS_ADDR                     UINT8_C(0x21)
#define BMI2_TEMPERATURE_0_ADDR                       UINT8_C(0x22)
#define BMI2_TEMPERATURE_1_ADDR                       UINT8_C(0x23)
#define BMI2_FIFO_LENGTH_0_ADDR                       UINT8_C(0x24)
#define BMI2_FIFO_DATA_ADDR                           UINT8_C(0x26)
#define BMI2_FEAT_PAGE_ADDR                           UINT8_C(0x2F)
#define BMI2_FEATURES_REG_ADDR                        UINT8_C(0x30)
#define BMI2_ACC_CONF_ADDR                            UINT8_C(0x40)
#define BMI2_ACC_RANGE_ADDR                           UINT8_C(0x41)	// Ajouté par Iouri
#define BMI2_GYR_CONF_ADDR                            UINT8_C(0x42)
#define BMI2_GYR_RANGE_ADDR                           UINT8_C(0x43)	// Ajouté par Iouri
#define BMI2_AUX_CONF_ADDR                            UINT8_C(0x44)
#define BMI2_FIFO_DOWNS_ADDR                          UINT8_C(0x45)
#define BMI2_FIFO_WTM_0_ADDR                          UINT8_C(0x46)
#define BMI2_FIFO_WTM_1_ADDR                          UINT8_C(0x47)
#define BMI2_FIFO_CONFIG_0_ADDR                       UINT8_C(0x48)
#define BMI2_FIFO_CONFIG_1_ADDR                       UINT8_C(0x49)
#define BMI2_SATURATION_ADDR                          UINT8_C(0x4A)
#define BMI2_AUX_DEV_ID_ADDR                          UINT8_C(0x4B)
#define BMI2_AUX_IF_CONF_ADDR                         UINT8_C(0x4C)
#define BMI2_AUX_RD_ADDR                              UINT8_C(0x4D)
#define BMI2_AUX_WR_ADDR                              UINT8_C(0x4E)
#define BMI2_AUX_WR_DATA_ADDR                         UINT8_C(0x4F)
#define BMI2_ERR_REG_MSK_ADDR                         UINT8_C(0x52)
#define BMI2_INT1_IO_CTRL_ADDR                        UINT8_C(0x53)
#define BMI2_INT2_IO_CTRL_ADDR                        UINT8_C(0x54)
#define BMI2_INT_LATCH_ADDR                           UINT8_C(0x55)
#define BMI2_INT1_MAP_FEAT_ADDR                       UINT8_C(0x56)
#define BMI2_INT2_MAP_FEAT_ADDR                       UINT8_C(0x57)
#define BMI2_INT_MAP_DATA_ADDR                        UINT8_C(0x58)
#define BMI2_INIT_CTRL_ADDR                           UINT8_C(0x59)
#define BMI2_INIT_ADDR_0                              UINT8_C(0x5B)
#define BMI2_INIT_ADDR_1                              UINT8_C(0x5C)
#define BMI2_INIT_DATA_ADDR                           UINT8_C(0x5E)
#define BMI2_INTERNAL_ERR_ADDR                        UINT8_C(0x5F)
#define BMI2_AUX_IF_TRIM                              UINT8_C(0x68)
#define BMI2_GYR_CRT_CONF_ADDR                        UINT8_C(0x69)
#define BMI2_NVM_CONF_ADDR                            UINT8_C(0x6A)
#define BMI2_IF_CONF_ADDR                             UINT8_C(0x6B)
#define BMI2_DRV_STR_ADDR                             UINT8_C(0x6C)
#define BMI2_ACC_SELF_TEST_ADDR                       UINT8_C(0x6D)
#define BMI2_GYR_SELF_TEST_AXES_ADDR                  UINT8_C(0x6E)
#define BMI2_SELF_TEST_MEMS_ADDR                      UINT8_C(0x6F)
#define BMI2_NV_CONF_ADDR                             UINT8_C(0x70)
#define BMI2_ACC_OFF_COMP_0_ADDR                      UINT8_C(0x71)
#define BMI2_GYR_OFF_COMP_3_ADDR                      UINT8_C(0x74)
#define BMI2_GYR_OFF_COMP_6_ADDR                      UINT8_C(0x77)
#define BMI2_GYR_USR_GAIN_0_ADDR                      UINT8_C(0x78)
#define BMI2_PWR_CONF_ADDR                            UINT8_C(0x7C)
#define BMI2_PWR_CTRL_ADDR                            UINT8_C(0x7D)
#define BMI2_CMD_REG_ADDR                             UINT8_C(0x7E)

/*! @name BMI2 I2C address */
#define BMI2_I2C_PRIM_ADDR                            UINT8_C(0x68)
#define BMI2_I2C_SEC_ADDR                             UINT8_C(0x69)

// ----------------------------------------------------------------------------
// Inlcusions de parties du fichier bmi270.c
// ----------------------------------------------------------------------------

/*! @name  Global array that stores the configuration file of BMI270 */
const uint8_t bmi270_config_file[] PROGMEM = {	// Le PROGMEM force le array à être stocké dans la flash
	
};	

#define IMU_ADDR	BMI2_I2C_PRIM_ADDR

static imu_error_e last_error = ERROR_NO_ERROR;

bool imu_init(){
	
	twi_init();
	
	twi_set_bitrate(BITRATE_400K);	// Tequila, 400 kbp, pas le temps de niaiser 	
	
	uint8_t register_value;
	
	_delay_ms(500);
	
	if(!twi_read_register_blocking(IMU_ADDR, BMI2_CHIP_ID_ADDR, &register_value, 1)){
		last_error = ERROR_INITIAL_COMMUNICATION;
		return false;
	}
	
	if(register_value != 0x24){		
		last_error = ERROR_INVALID_CHIP_ID;
		return false;
	}
	
	// Désactivation du mode power save
	register_value = 0x00;
	if(!twi_write_register_blocking(IMU_ADDR,  BMI2_PWR_CONF_ADDR, &register_value, 1)){
		last_error = ERROR_ONGOING_COMMUNICATION;
		return false;
	}
	
	// Selon la datasheet (https://www.bosch-sensortec.com/media/boschsensortec/downloads/datasheets/bst-bmi270-ds000.pdf) à la p. 18,
	// il faut minimalement attendre 450 us
	
	_delay_ms(1); // 1 ms = 1000 us > 450 us
	
	// Il n'est pas impossible que le chip soit déjà initialisé, si c'est le cas, il ne faut pas reloader le firmware
	if(!twi_read_register_blocking(IMU_ADDR, BMI2_INTERNAL_STATUS_ADDR, &register_value, 1)){
		last_error = ERROR_READ_STATUS_FAILED;
		return false;
	}
	
	bool skip_config_load = false;
	
	if(register_value == 0x01){
		skip_config_load = true;
	}
	
	if(!skip_config_load){	// Si le chip n'était pas déjà initialisé
		
		// Préparation de la data d'initialisation
		register_value = 0x00;
		if(!twi_write_register_blocking(IMU_ADDR,  BMI2_INIT_CTRL_ADDR, &register_value, 1)){
			last_error = ERROR_ONGOING_COMMUNICATION;
			return false;
		}
	
		// Préparation de la data d'initialisation
		if(!twi_write_register_blocking_progmem(IMU_ADDR,  BMI2_INIT_DATA_ADDR, bmi270_config_file, sizeof(bmi270_config_file))){
			last_error = ERROR_ONGOING_COMMUNICATION;
			return false;
		}
	
		// Complétion du config load
		register_value = 0x01;
		if(!twi_write_register_blocking(IMU_ADDR,  BMI2_INIT_CTRL_ADDR, &register_value, 1)){
			last_error = ERROR_ONGOING_COMMUNICATION;
			return false;
		}
	
		// On attend que le chip soit initialisé
		uint8_t timeout_ms = 30;
		while(true){	// Tant que le internal status retourne "ASIC is not initialized"
			if(!twi_read_register_blocking(IMU_ADDR, BMI2_INTERNAL_STATUS_ADDR, &register_value, 1)){
				last_error = ERROR_ONGOING_COMMUNICATION;
				return false;
			}
		
			if(register_value == 0x01){
				break;
			}
		
			timeout_ms--;
			if(timeout_ms == 0){
				last_error = ERROR_INIT_TIMEOUT_BUSTED;
				return false;
			}
		
			_delay_ms(1);
		}
	}
	
	// On active l'accéléromètre et le gyroscope
	register_value = 0b00000110;	// bit 3 (temperature): disable, bit 2 (accelerometer): enable, bit 1 (gyroscope): enable, bit 0 (auxiliary): disable
	if(!twi_write_register_blocking(IMU_ADDR,  BMI2_PWR_CTRL_ADDR, &register_value, 1)){
		last_error = ERROR_ONGOING_COMMUNICATION;;
		return false;
	}
	
	register_value = 0b00000010;	// Je sais pas
	if(!twi_write_register_blocking(IMU_ADDR,  BMI2_PWR_CONF_ADDR, &register_value, 1)){
		last_error = ERROR_ONGOING_COMMUNICATION;;
		return false;
	}
	
	// On met une configuration par défaut
	if(!imu_set_accelerometer_config(DATA_RATE_25HZ, ACCELERATION_RANGE_8G)){
		return false;
	}
	
	if(!imu_set_gyroscope_config(DATA_RATE_25HZ, GYROSCOPE_RANGE_2000DPS)){
		return false;
	}
	
	// On fait une première lecture
	if(!imu_refresh()){
		return false;
	}
	
	// Cette prochaine étape peu sembler louche, voici donc son explication
	// Pour une raison que j'ignore, des fois l'accéléromètre et le gyroscope retournent une lecture nulle juste après l'initialisation
	// Même complètement immobile et parfaitement à plat, l'accéléromètre devrait toujours retourner un petit bruit de quelques LSB
	// Si la lecture donne 0, je lui laisse une autre chance et après j'abandonne
	if(imu_get_acceleration_x() == 0){
		
		uint8_t timeout_ms = 60;	// Le 17 octobre j'ai mesuré le délais à 44 ms donc 60 devrait être safe
		while(!imu_is_new_data_ready()){
			
			timeout_ms--;
			if(timeout_ms == 0){
				last_error = ERROR_DATA_READY_TIMEOUT_BUSTED;
				return false;
			}
			_delay_ms(1);
		}
		
		// On fait une deuxième lecture
		if(!imu_refresh()){
			return false;
		}
		
		// Rendu ici il ne faut plus que ça donne 0
		if(imu_get_acceleration_x() == 0){
			last_error = ERROR_DATA_IS_INVALID;
			return false;
		}
	}
	
	return true;	// Si on se rend ici, l'initialisation a réussi
}

bool imu_set_accelerometer_config(imu_data_rate_e data_rate, imu_accelerometer_range_e accelerometer_range){
	
	// Définition des bits des registres
	const int ACC_ODR = 0;
	const int ACC_ODR_MASK = 0b00001111;
	const int ACC_BWP = 4;
	const int ACC_BWP_MASK = 0b01110000;
	const int ACC_FILTER_PERF = 7;
	const int ACC_FILTER_PERF_MASK = 0b10000000;
	const int ACC_RANGE = 0;
	const int ACC_RANGE_MASK = 0b00000011;
	
	uint8_t register_value[2];
	
	// Voir datasheet (https://www.bosch-sensortec.com/media/boschsensortec/downloads/datasheets/bst-bmi270-ds000.pdf) p.101
	register_value[0] = ((data_rate << ACC_ODR) & ACC_ODR_MASK) | ((0x02 << ACC_BWP) & ACC_BWP_MASK) | ((0x01 << ACC_FILTER_PERF) & ACC_FILTER_PERF_MASK);	// data rate, filtre normal, optimization de la performance
	
	// Voir datasheet (https://www.bosch-sensortec.com/media/boschsensortec/downloads/datasheets/bst-bmi270-ds000.pdf) p.102
	register_value[1] = (accelerometer_range << ACC_RANGE) & ACC_RANGE_MASK;	// le range de l'accélérmoètre	
	
	if(!twi_write_register_blocking(IMU_ADDR,  BMI2_ACC_CONF_ADDR, register_value, 2)){
		last_error = ERROR_ACCELEROMETER_CONFIG_FAILED;
		return false;
	}
	
	return true;
}

bool imu_set_gyroscope_config(imu_data_rate_e data_rate, imu_gyroscope_range_e gyroscope_range){
	
	// Définition des bits des registres
	const int GYR_ODR = 0;
	const int GYR_ODR_MASK = 0b00001111;
	const int GYR_BWP = 4;
	const int GYR_BWP_MASK = 0b00110000;
	const int GYR_NOISE_PERF = 6;
	const int GYR_NOISE_PERF_MASK = 0b01000000;
	const int GYR_FILTER_PERF = 7;
	const int GYR_FILTER_PERF_MASK = 0b10000000;
	const int GYR_RANGE = 0;
	const int GYR_RANGE_MASK = 0b00000111;
	const int OIS_RANGE = 3;
	const int OIS_RANGE_MASK = 0b00001000;
	
	uint8_t register_value[2];
	
	// Voir datasheet (https://www.bosch-sensortec.com/media/boschsensortec/downloads/datasheets/bst-bmi270-ds000.pdf) p.102
	register_value[0] = ((data_rate << GYR_ODR) & GYR_ODR_MASK) | ((0x02 << GYR_BWP) & GYR_BWP_MASK) | ((0x01 << GYR_NOISE_PERF) & GYR_NOISE_PERF_MASK) | ((0x01 << GYR_FILTER_PERF) & GYR_FILTER_PERF_MASK);	// data rate, filtre normal, bruit optimisé, filtre optimisé
	
	// Voir datasheet (https://www.bosch-sensortec.com/media/boschsensortec/downloads/datasheets/bst-bmi270-ds000.pdf) p.103
	register_value[1] = ((gyroscope_range << GYR_RANGE) & GYR_RANGE_MASK) | ((0x00 << OIS_RANGE) << OIS_RANGE_MASK);	// Le range de l'accéléromètre et la valeur par défaut pour l'OIS parce qu'on s'en fou
	
	if(!twi_write_register_blocking(IMU_ADDR,  BMI2_GYR_CONF_ADDR, register_value, 2)){
		last_error = ERROR_GYROSCOPE_CONFIG_FAILED;
		return false;
	}
		
	return true;	
}

bool imu_set_data_ready_interrupt(bool interrupt_enabled){
	
	uint8_t register_value;
	
	// Voir datasheet (https://www.bosch-sensortec.com/media/boschsensortec/downloads/datasheets/bst-bmi270-ds000.pdf) p.109
	register_value = interrupt_enabled? 0b00001010: 0b00000000; // Si enabled : active high, push-pull, output enabled, input disabled 
	if(!twi_write_register_blocking(IMU_ADDR,  BMI2_INT1_IO_CTRL_ADDR, &register_value, 1)){
		last_error = ERROR_INTERRUPT_CONFIG_FAILED;
		return false;
	}
		
	// Voir datasheet (https://www.bosch-sensortec.com/media/boschsensortec/downloads/datasheets/bst-bmi270-ds000.pdf) p.112
	register_value = interrupt_enabled? (1 << 2): 0;
	if(!twi_write_register_blocking(IMU_ADDR,  BMI2_INT_MAP_DATA_ADDR, &register_value, 1)){
		last_error = ERROR_INTERRUPT_CONFIG_FAILED;
		return false;
	}
		
	return true;
}

bool imu_is_new_data_ready(){
	
	last_error = ERROR_NO_ERROR;
	
	// Définition des bits du registre
	// const int AUX_BUSY_MASK	= 0b00000100;
	// const int CMD_RDY_MASK	= 0b00010000;
	// const int DRDY_AUX_MASK	= 0b00100000;
	const int DRDY_GYR_MASK		= 0b01000000;
	const int DRDY_ACC_MASK		= 0b10000000;
	
	uint8_t register_value;
	
	if(!twi_read_register_blocking(IMU_ADDR, BMI2_STATUS_ADDR, &register_value, 1)){
		last_error = ERROR_READ_STATUS_FAILED;
		return false;
	}
	
	if(register_value & (DRDY_GYR_MASK | DRDY_ACC_MASK)){	// S'il y a un 1 dans le bit "data ready gyroscope" ou "data ready accelerometer"
		return true;
	}	
	
	return false;
}

static int16_t acceleration_x = 0;
static int16_t acceleration_y = 0;
static int16_t acceleration_z = 0;
static int16_t angular_velocity_x = 0;
static int16_t angular_velocity_y = 0;
static int16_t angular_velocity_z = 0;

bool imu_refresh(){
	
	uint8_t buffer[12];
	
	// On lit les 12 registres consécutifs en partant de l'accélération en X
	if(!twi_read_register_blocking(IMU_ADDR, BMI2_ACC_X_LSB_ADDR, buffer, 12)){
		last_error = ERROR_READ_STATUS_FAILED;
		return false;
	}
	
	uint16_t lsb, msb;
	
	// Accélération en x
	lsb = buffer[0];
	msb = buffer[1];
	
	acceleration_x = lsb | (msb << 8);
	
	// Accélération en y
	lsb = buffer[2];
	msb = buffer[3];
		
	acceleration_y = lsb | (msb << 8);
	
	// Accélération en z
	lsb = buffer[4];
	msb = buffer[5];
		
	acceleration_z = lsb | (msb << 8);
	
	// Vitesse angulaire en x
	lsb = buffer[6];
	msb = buffer[7];
	
	angular_velocity_x = lsb | (msb << 8);
	
	// Vitesse angulaire en y
	lsb = buffer[8];
	msb = buffer[9];
	
	angular_velocity_y = lsb | (msb << 8);
	
	// Vitesse angulaire en z
	lsb = buffer[10];
	msb = buffer[11];
	
	angular_velocity_z = lsb | (msb << 8);
	
	return true;
}

int16_t imu_get_acceleration_x(){
	return acceleration_x;
}

int16_t imu_get_acceleration_y(){
	return acceleration_y;
}

int16_t imu_get_acceleration_z(){
	return acceleration_z;
}

int16_t imu_get_angular_velocity_x(){
	return angular_velocity_x;
}

int16_t imu_get_angular_velocity_y(){
	return angular_velocity_y;
}

int16_t imu_get_angular_velocity_z(){
	return angular_velocity_z;
}

imu_error_e imu_get_last_error(){
	return last_error;
}